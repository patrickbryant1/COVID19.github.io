...
def read_and_format_data(datadir, countries):
        ...
        #Covariate names
        covariate_names = ['schools_universities', 'self_isolating_if_ill','public_events', 'any_intervention', 'social_distancing_encouraged']
        #Get data by country
...
def simulate(stan_data):
        ...
        sm =  pystan.StanModel(file='nolockdown.stan')
...
def visualize_results(outdir, countries, covariate_names, dates_by_country, deaths_by_country, cases_by_country):
    ...
    ax.set_ylabel('Count')
    ax.set_xlabel("Rhat")
    fig.savefig(outdir+subdir+'plots/rhat.png', format='png')
    plt.close()
    ...
    plt.tight_layout()
    fig.savefig(outdir+subdir+'plots/alphas.png', format='png')
    plt.close()
    ...
    for i in range(len(countries)):
        ...
        plot_shade_ci(days[:end], dates, case_av, observed_country_cases, case_std,'Cases',outdir+subdir+'plots/'+country+'_cases.png')
        ...
        plot_shade_ci(days[:end],dates,death_av,observed_country_deaths, death_std,'Deaths',outdir+subdir+'plots/'+country+'_deaths.png')

        #Plot R
        country_Rt = Rt[:,:,i][2000:,:end]
        Rt_av =  np.average(country_Rt,axis=0)
        Rt_std =  np.std(country_Rt,axis=0)
        plot_shade_ci(days[:end],dates,Rt_av,'', Rt_std,'Rt',outdir+subdir+'plots/'+country+'_Rt.png')

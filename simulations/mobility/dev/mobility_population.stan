data {
  ...
  int pop_frac_age[10,M]; //Population fractions per age in steps of 10 from 0 to 90+
  int death_frac_age[10,M]; //Population fractions per age in steps of 10 from 0 to 90+
...
parameters {
  ...
  real<lower=0> alpha_scale1[M]; // Rt^exp(sum(alpha))
  real<lower=0> alpha_scale2[M]; // Rt^exp(sum(alpha))
  real<lower=0> alpha_scale3[M]; // Rt^exp(sum(alpha))
  real<lower=0> alpha_scale4[M]; // Rt^exp(sum(alpha))
  real<lower=0> alpha_scale5[M]; // Rt^exp(sum(alpha))
...
transformed parameters {
  ...
  //For covariate 5 (residential), the opposite is true. More mobility at home --> less spread. Why the sign is negative.
        Rt[,m] = mu[m] * exp(covariate1[,m] * alpha_scale1[m] * (alpha[1]) + covariate2[,m] * alpha_scale2[m] * (alpha[2]) +
        covariate3[,m] * alpha_scale3[m] * (alpha[3])+ covariate4[,m] * alpha_scale4[m] * (alpha[4]) - covariate5[,m] * alpha_scale5[m] * (alpha[5]));
...
model {
  ...
  alpha_scale1 ~ uniform(0.,1.); #Make the relative scaling constant, uniform btw 0 and 1
  alpha_scale2 ~ uniform(0.,1.); #Make the relative scaling constant, uniform btw 0 and 1
  alpha_scale3 ~ uniform(0.,1.); #Make the relative scaling constant, uniform btw 0 and 1
  alpha_scale4 ~ uniform(0.,1.); #Make the relative scaling constant, uniform btw 0 and 1
  alpha_scale5 ~ uniform(0.,1.); #Make the relative scaling constant, uniform btw 0 and 1

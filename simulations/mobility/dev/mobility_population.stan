...
transformed parameters {
    ...
    matrix[N2, M] prediction[10]; //predict for each day for all countries
    ...
    //For covariate 5 (residential), the opposite is true. More mobility at home --> less spread. Why the sign is negative.
      ...
      //Cases
        ...
        for(j in 1:(i-1)) {
          convolution += prediction[p,j,m]*SI[i-j]*pop_frac_age[p,m]; //Cases today due to cumulative probability, sum(cases*rel.change due to SI)
          }
